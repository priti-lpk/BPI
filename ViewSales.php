...
if (isset($_GET['type']) && isset($_GET['id'])) {
    $edba = new DBAdapter();
    $edata = $edba->getRow("sales_list", array("*"), "id=" . $_GET['id']);
    echo "<input type='hidden' id='sales_id' value='" . $_GET['id'] . "'>";
}
                                            ...
                                            <div class="col-md-12">
                                                <div class="widget widget-table">
                                                    <div class="widget-header">
                                                        <h3><i class="fa fa-table"></i>View Sales</h3>
                                                    </div>
...
//                                                                $id = $_GET['party_id'];
                                                                ...
                                                                include_once("shreeLib/dbconn.php");
                                                                $sql = "SELECT sales_list.id,party_list.party_name,sales_list.total_amount,sales_list.pay_type,sales_list.note, sales_list.user_id FROM sales_list INNER JOIN party_list ON party_list.id = sales_list.party_id WHERE sales_list.sale_date >='" . $firstdate . "' AND sales_list.sale_date <='" . $lastdate . "' and party_list.branch_id=" . $last_id;
                                                                $resultset = mysqli_query($con, $sql);
...
//        var my_object = {"s_id": party_id, "f_date": f_date, "l_date": l_date, "dt_data": detedata, "ed_data": editdata};
//
//        $.ajax({
//            url: 'getViewData.php',
//            dataType: "html",
    ...
    function SetForDelete(id) {
        swal({
            ...
            closeOnCancel: false
                ...
                function (isConfirm) {
                    if (isConfirm) {
                        location.href = "Delete.php?type=sales_list&id=" + id;
