...
if (isset($_GET['type']) && isset($_GET['id'])) {
    ...
    $field = array("*");
    $datas = $edba->getRow("states", $field, "id=" . $_GET['id']);
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="card m-b-20">
                                        <div class="card-body">                   
                                            <form action="customFile/statesPro.php" id="form_data" class="form-horizontal" role="form" method="post" enctype="multipart/form-data" >  
...
//                                                    $data = $dba->getRow("countries", array("id", "name"), "1");
                                                            foreach ($result as $subData) {
                                                                echo "<option " . ($subData['id'] == $datas[0][1] ? 'selected' : '') . " value='" . $subData['id'] . "'>" . $subData['cname'] . "</option> ";
                                                            }
                                                            ?>
                                                        </select>
                                                    </div>
                                                ...
                                                <div class="form-group row">
                                                    ...
                                                    <div class="col-sm-10">
                                                        <input class="form-control" type="text"  placeholder="State Name" id="state_name" name="state_name" value="<?php echo (isset($_GET['type']) && isset($_GET['id']) ? $datas[0][2] : ''); ?>" required="">
                                                    </div>
                                                </div>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                                ...
                                                <tbody>
                                                    ...
                                                    include_once 'shreeLib/DBAdapter.php';
                                                    $dba = new DBAdapter();
                                                    $field = array("states.id,countries.cname,states.state_name");
                                                    $data = $dba->getRow("states INNER JOIN countries ON states.country_id=countries.id", $field, "1");
                                                    $count = count($data);
                                                    $k = 1;
                                                    if ($count >= 1) {
                                                        foreach ($data as $subData) {
                                                            ...
                                                            echo "<td><a href='states.php?type=edit&id=" . $subData[0] . "' class='btn btn-primary waves-effect waves-light'>Edit</a> <button class='btn btn-danger btn_delete' id='" . $subData[0] . "' onclick='SetForDelete(this.id);'><i class='fa fa-trash-o'>  Delete</button></td>";
        ...
        <script type="text/javascript">
            ...
            function SetForDelete(id) {
                location.href = "Delete.php?type=state&id=" + id;
